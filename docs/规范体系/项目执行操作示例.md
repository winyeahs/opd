...
graph TB
    L1[用户界面层]
    C1[TodoList Component]
    C2[TodoItem Component]
    C3[TodoForm Component]
    C4[FilterBar Component]
    C5[SearchBar Component]
    C6[Header Component]
    
    L2[状态管理层]
    S[Zustand Store<br/>todoStore<br/>todos: Todo[]<br/>filter: FilterOptions<br/>actions: add, update, delete, toggle]
    
    L3[数据持久层]
    D[LocalStorage Service<br/>saveTodos(todos)<br/>loadTodos]
